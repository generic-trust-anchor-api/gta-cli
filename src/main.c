...
static char profiles_to_register[][MAXLEN_PROFILE] = {"ch.iec.30168.basic.local_data_protection",
...
bool basic_sw_provider_gta_register_provider(gta_instance_handle_t h_inst, gtaio_istream_t * init_config,
        ...
        if (strncmp(argv[i], "--app_name=", 11) == 0) {
            ...
            */
            ...
            while (fgets(str_temp, 150, p_file_attributes)) {
                ...
                if (0 != strlen(str_temp)) {
                    ...
                    if (NULL != p_new_attribute) {
                        ...
                        if (EXIT_SUCCESS !=
                            parse_attributes(str_temp,
                                             &(arguments->ctx_attributes.p_attr[arguments->ctx_attributes.num - 1]))) {
                            fprintf(stderr, "Missing function arguments\n");
                            return EXIT_FAILURE;
                        }
                    } else {
            ...
            if (NULL != p_new_attribute) {
                ...
                if (EXIT_SUCCESS !=
                    parse_attributes(argv[i],
                                     &(arguments->ctx_attributes_bin.p_attr[arguments->ctx_attributes_bin.num - 1]))) {
                    fprintf(stderr, "Missing function arguments\n");
                    return EXIT_FAILURE;
                }
            } else {
...
int main(int argc, char * argv[])
{
    ...
    case unseal_data: {
        ...
        if (!gta_unseal_data(h_ctx, (gtaio_istream_t *)&istream_sealed_data, (gtaio_ostream_t *)&ostream_unsealed_data,
                             &errinfo)) {
    ...
    case personality_enumerate: {
        ...
        while (b_loop) {
            ...
            if (gta_personality_enumerate(h_inst, arguments.id_val, &h_enum, pers_flag, (gtaio_ostream_t *)&o_persname,
                                          &errinfo)) {
                printf("[%d]\n", num_of_personality);
                printf("Identifier Value:   %s\n", arguments.id_val);
    ...
    case personality_enroll: {
        ...
        if (0 < arguments.ctx_attributes_bin.num) {
            ...
            for (size_t i = 0; i < arguments.ctx_attributes_bin.num; i++) {
                if (!myio_open_ifilestream(&ifilestream_attr_val, arguments.ctx_attributes_bin.p_attr[i].p_val,
                                           &errinfo)) {
                    printf("Cannot open file %s\n", arguments.ctx_attributes_bin.p_attr[i].p_val);
                    return EXIT_FAILURE;
                }

                if (!gta_context_set_attribute(h_ctx, arguments.ctx_attributes_bin.p_attr[i].p_type,
                                               (gtaio_istream_t *)&ifilestream_attr_val, &errinfo)) {
        ...
        if (0 < arguments.ctx_attributes.num) {
            ...
            for (size_t i = 0; i < arguments.ctx_attributes.num; i++) {
                istream_from_buf_init(&istream_attr_val, arguments.ctx_attributes.p_attr[i].p_val,
                                      strlen(arguments.ctx_attributes.p_attr[i].p_val) + 1);

                if (!gta_context_set_attribute(h_ctx, arguments.ctx_attributes.p_attr[i].p_type,
                                               (gtaio_istream_t *)&istream_attr_val, &errinfo)) {
