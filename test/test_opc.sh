...
if ! test -r "${TEST_DIRECTORY}/ecc_CA.key" || ! test -r "${TEST_DIRECTORY}/ecc_CA.crt"
then 
echo "create dummy CA key and dummy CA certificate" 
echo "openssl req -x509 -new -newkey ec:<(openssl ecparam -name secp521r1) -keyout ${TEST_DIRECTORY}/ecc_CA.key -out ${TEST_DIRECTORY}/ecc_CA.crt -nodes -subj "/CN=Dummy CA /O=Dummy Org/C=DE" -days 365"
openssl req -x509 -new -newkey ec:<(openssl ecparam -name secp521r1) -keyout "${TEST_DIRECTORY}/ecc_CA.key" -out "${TEST_DIRECTORY}/ecc_CA.crt" -nodes -subj "/CN=Dummy CA /O=Dummy Org/C=DE" -days 365
...
openssl x509 -in "${TEST_DIRECTORY}/ldevid_ee.crt" -text
...
echo "openssl x509 -req -in ${TEST_DIRECTORY}/test1.csr -inform DER -out ${TEST_DIRECTORY}/test1.crt -CA ${TEST_DIRECTORY}/ecc_CA.crt -CAkey ${TEST_DIRECTORY}/ecc_CA.key -CAcreateserial -days 9125 -extensions v3_req -copy_extensions copyall"
openssl x509 -req -in "${TEST_DIRECTORY}/test1.csr" -inform DER -out "${TEST_DIRECTORY}/test1.crt" -CA "${TEST_DIRECTORY}/ecc_CA.crt" -CAkey "${TEST_DIRECTORY}/ecc_CA.key" -CAcreateserial -days 9125 -extensions v3_req -copy_extensions copyall
assert_success "openssl x509 -req"
openssl x509 -in "${TEST_DIRECTORY}/test1.crt" -text
assert_success "openssl_x509_-in"
echo ""
# todo: check if subject and subjectAltName in certificate is like expected
...
echo "openssl x509 -req -in ${TEST_DIRECTORY}/test2.csr -inform DER -out ${TEST_DIRECTORY}/test2.crt -CA ${TEST_DIRECTORY}/ecc_CA.crt -CAkey ${TEST_DIRECTORY}/ecc_CA.key -CAcreateserial -days 9125 -extensions v3_req -copy_extensions copyall"
openssl x509 -req -in "${TEST_DIRECTORY}/test2.csr" -inform DER -out "${TEST_DIRECTORY}/test2.crt" -CA "${TEST_DIRECTORY}/ecc_CA.crt" -CAkey "${TEST_DIRECTORY}/ecc_CA.key" -CAcreateserial -days 9125 -extensions v3_req -copy_extensions copyall
assert_success "openssl_x509_-req"
openssl x509 -in "${TEST_DIRECTORY}/test2.crt" -text
assert_success "openssl_x509_-in"
echo ""
# todo: check if subject and subjectAltName in certificate is like expected
...
echo "get public key"
openssl x509 -pubkey -noout -in "${TEST_DIRECTORY}/ldevid_ee.crt"  > "${TEST_DIRECTORY}/ldevid_ee_pub_key.pem"
